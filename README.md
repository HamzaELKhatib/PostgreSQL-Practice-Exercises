...
```
WITH rental_languages AS (SELECT f.title, l.name, l.language_id, sum(p.amount) AS revenue
                          FROM film f
                                   JOIN language l USING (language_id)
                                   JOIN inventory i USING (film_id)
                                   JOIN rental r USING (inventory_id)
     ...
     rental_times AS (SELECT rl.title,
                             rl.name,
                             rl.revenue,
                             ROW_NUMBER() OVER (PARTITION BY rl.name ORDER BY rl.revenue DESC) as rank
                      FROM rental_languages rl)
SELECT title, name, revenue
FROM rental_times
WHERE rank < 6
ORDER BY rank
```
